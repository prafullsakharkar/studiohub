  ...
  useEffect(() => {
    if (org) {
      setFormData({
        ...
        code: org.code || '',
        slug: org.slug || '',
        tier: org.tier || 'Enterprise Vanguard',
        status: org.status || 'Active',
        headquarters: org.headquarters || '',
        primary_contact_name: org.primary_contact_name || '',
        primary_contact_email: org.primary_contact_email || '',
        crew_count: org.crew_count || 0,
        offices_count: org.offices_count || 1,
        storage_quota_tb: org.storage_quota_tb || 250,
        ...
        render_farm_region: org.settings?.render_farm_region ?? '',
  ...
  if (isLoading) {
    return (
      <div className="p-12 text-center text-xs text-slate-400 font-mono">
        Loading organization specifications...
      </div>
    );
  }
  ...
  if (!org && !isLoading) {
    return (
      <div className="p-12 text-center text-xs text-rose-400 font-mono">
        Studio Organization with ID "{id}" was not found.
      </div>
    );
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-white tracking-tight">Edit Studio Profile</h1>
              <span className="font-mono text-xs px-2 py-0.5 rounded bg-indigo-950/80 text-indigo-300 border border-indigo-500/30">
                {org?.code}
              </span>
            </div>
            <p className="text-xs text-slate-400">
              Update organization metadata, tenancy tier, pipeline presets, and security policies.
            </p>
          </div>
        </div>
          ...
          <div className="p-5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-5">
            <div>
              <h2 className="text-sm font-bold text-white flex items-center gap-2">
                ...
                Studio Profile & Operational Scope
              </h2>
              <p className="text-xs text-slate-400">
                Primary company information and tenant isolation identifiers.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">
                  Studio Code <span className="text-rose-400">*</span>
                </label>
                <input
                  type="text"
                  required
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({ ...formData, status: e.target.value as StudioStatus })}
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-xs text-slate-200 focus:outline-hidden focus:border-indigo-500"
                >
                  <option value="Active">Active (Full Production Pipeline)</option>
                  <option value="Onboarding">Onboarding</option>
                  <option value="Suspended">Suspended</option>
                  <option value="Archived">Archived (Read-Only Storage)</option>
                </select>
              </div>
            ...
            <div className="pt-4 border-t border-slate-800/80 grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Crew Count</label>
                <input
                  type="number"
                  min={1}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Facilities / Offices Count</label>
                <input
                  type="number"
                  min={1}
          ...
          <div className="p-5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-5">
            <div>
              <h2 className="text-sm font-bold text-white flex items-center gap-2">
                ...
                Studio Visual Identity & Portal Assets
              </h2>
              <p className="text-xs text-slate-400">
                Custom iconography and banners shown across screening rooms and client review portals.
              </p>
            </div>
          ...
          <div className="p-5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-5">
            <div>
              <h2 className="text-sm font-bold text-white flex items-center gap-2">
                ...
                Pipeline Standards, OpenUSD & OCIO Config
              </h2>
              <p className="text-xs text-slate-400">
                Default rendering, color pipeline, and Hydra schema versions.
              </p>
            </div>
        ...
        {activeSection === 'security' && (
          <div className="p-5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-5">
            <div>
              ...
              <p className="text-xs text-slate-400">
                MPAA and CDSA studio security accreditation controls.
              </p>
            </div>
              ...
              <label className="flex items-center justify-between p-3.5 rounded-lg bg-slate-950/60 border border-slate-800 cursor-pointer hover:bg-slate-950/90">
                <div>
                  ...
                  <div className="text-[11px] text-slate-400">
                    Mandate hardware keys or authenticator TOTP apps for all studio accounts.
                  </div>
                </div>
                <input
                  type="checkbox"
