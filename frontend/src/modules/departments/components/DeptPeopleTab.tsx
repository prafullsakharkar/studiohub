...
export const DeptPeopleTab: React.FC<{ dept: DepartmentEntity }> = ({ dept }) => {
  const { data: peopleData } = usePeople();
  const people = (peopleData?.results || []).filter(
    (p) => p.department_id === dept.id || p.department_name === dept.name
  );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-3 border-b border-slate-800">
        <div>
          <h3 className="text-sm font-bold text-white flex items-center gap-2">
            ...
            Department Roster & Active Crew
          </h3>
          <p className="text-xs text-slate-400">
            Artists, technical directors, and leads belonging to {dept.name}.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {people.length === 0 ? (
          <div className="col-span-3 p-8 text-center rounded-xl border border-dashed border-slate-800 text-slate-500 text-xs">
            No artists assigned to this department yet.
          </div>
        ) : (
          people.map((person) => (
            <div
