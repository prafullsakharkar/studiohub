...
export const DeptProjectsTab: React.FC<{
  dept: DepartmentEntity;
  onAssignProject?: (projectCode: string) => void;
  onRemoveProject?: (projectCode: string) => void;
}> = ({ dept, onAssignProject, onRemoveProject }) => {
  const { data: projectsData } = useProjects();
  const { openTab } = useWorkspaceStore();
  const navigate = useNavigate();

  const assigned = dept.assigned_projects || ['NK99', 'CR88'];
      ...
      <div className="flex items-center justify-between pb-3 border-b border-slate-800">
        <div>
          <h3 className="text-sm font-bold text-white flex items-center gap-2">
            ...
            Active Production Shows with Department Bids
          </h3>
          <p className="text-xs text-slate-400">
            Shows where {dept.name} holds active shot quotas, asset builds, or review pipelines.
          </p>
        </div>
      </div>
