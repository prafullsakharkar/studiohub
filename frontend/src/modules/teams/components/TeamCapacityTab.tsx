...
export const TeamCapacityTab: React.FC<{ team: Team }> = ({ team }) => {
    const weeklyCapacity = team.member_count * 40;
    const currentUtilization = team.capacity_utilization || 78;
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                <div>
                    <h3 className="text-sm font-bold text-white flex items-center gap-2">
                        ...
                        Squad Capacity Planning & Quota Allocation
                    </h3>
                    <p className="text-xs text-slate-400">
                        Total man-hours available across {team.member_count} squad members.
                    </p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="rounded-xl border border-slate-800 bg-slate-900/80 p-4">
                    ...
                    <span className="text-xs text-slate-400 mt-1 block">{team.member_count} members × 40 hrs base</span>
                </div>
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-5 space-y-3">
                <div className="flex items-center justify-between text-xs">
                    <span className="font-mono text-slate-400">Squad Workload vs Limit</span>
                    <span className="font-mono text-white font-bold">{bookedHours}h / {weeklyCapacity}h</span>
                </div>
                <div className="w-full bg-slate-950 h-3 rounded-full overflow-hidden border border-slate-800">
                    <div
                        className={`h-full rounded-full transition-all ${currentUtilization > 90 ? 'bg-rose-500' : currentUtilization > 75 ? 'bg-indigo-500' : 'bg-emerald-500'
                            }`}
