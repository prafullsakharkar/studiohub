...
export const TeamProjectsTab: React.FC<{
    team: Team;
    onAssignProject?: (projectCode: string) => void;
    onRemoveProject?: (projectCode: string) => void;
}> = ({ team, onAssignProject, onRemoveProject }) => {
    const { data: projectsData } = useProjects();
    const { openTab } = useWorkspaceStore();
    const navigate = useNavigate();

    const assigned = team.assigned_projects || [team.current_project_code];
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                <div>
                    <h3 className="text-sm font-bold text-white flex items-center gap-2">
                        ...
                        Active Production Shows with Squad Bids
                    </h3>
                    <p className="text-xs text-slate-400">
                        Shows where {team.name} holds active shot quotas, asset builds, or review pipelines.
                    </p>
                </div>
            </div>
                            ...
                            <div>
                                <div className="flex items-start justify-between">
                                    ...
                                    <Badge variant={isAssigned ? 'success' : 'outline'} className="text-[10px] font-mono">
                                        {isAssigned ? 'Active Scope' : 'No Scope'}
                                    </Badge>
                                </div>
