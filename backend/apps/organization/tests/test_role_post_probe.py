...
def test_probe_role_crud():
    ...
    Permission.objects.get_or_create(code="project.view", defaults={"name": "pr", "module": "projects", "action": "read"})
    Permission.objects.get_or_create(code="shot.view", defaults={"name": "sr", "module": "shots", "action": "read"})
    client = APIClient()
    client.force_authenticate(user=user)
    hdr = {"HTTP_X_ORGANIZATION_ID": str(org.id)}
    r = client.post("/api/v1/roles/", {"name": "Probe Role", "permissions": ["project.view"]}, format="json", **hdr)
    print("\nCREATE ->", r.status_code, str(getattr(r, "data", ""))[:200])
    if r.status_code == 201:
        rid = r.data["id"]
        r2 = client.patch(f"/api/v1/roles/{rid}/", {"permissions": ["shot.view"]}, format="json", **hdr)
