...
class ClientContactViewSet(NestedBulkActionsMixin, OrganizationEntityViewSet):  # pyright: ignore[reportMissingTypeArgument]
    ...
    parent_field = "client"
    detail_serializer_class = ClientContactDetailSerializer
    create_serializer_class = ClientContactCreateSerializer
    update_serializer_class = ClientContactUpdateSerializer
    ...
    def _get_parent_client(self):
        ...
        a user can never attach contacts to another organization's client.
...
class VendorContactViewSet(NestedBulkActionsMixin, OrganizationEntityViewSet):  # pyright: ignore[reportMissingTypeArgument]
    ...
    parent_field = "vendor"
    detail_serializer_class = VendorContactDetailSerializer
    create_serializer_class = VendorContactCreateSerializer
    update_serializer_class = VendorContactUpdateSerializer
    ...
    def _get_parent_vendor(self):
        ...
        Always scoped to the request's organization (including for staff):
        a user can never attach contacts to another organization's vendor.
        Without an organization context the parent does not resolve (404).
        """
        vendor_pk = self.kwargs.get("vendor_pk")
        if not vendor_pk:
