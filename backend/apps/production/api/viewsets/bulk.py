"""
Shared bulk-action endpoints for project-scoped production entities.

``BulkContractViewSetMixin`` exposes the bulk surface required by the
frontend contract while preserving the legacy backend response shapes:

- ``POST check-existence/`` (new) + ``POST existence-check/`` (kept):
  accepts ``{project_id, codes[]}`` (frontend) or ``{items:[{project_id,
  code}]}`` (legacy); responds with **both** ``items[]`` (frontend
  ``CheckExistenceResponse``: ``{code, state, existing_entity?, message?}``)
  and ``results[]`` (legacy ``ExistenceResponse``: ``{index, status, ...}``).
- ``POST bulk-create/``: accepts frontend
  ``{project_id, items:[{code, action, data, existing_id?}]}`` or legacy
  ``{items:[flat...]}``; responds with a union of the frontend
  ``BulkOperationResponse`` and the legacy ``BulkResponse``.
- ``POST|PATCH bulk-update/``: accepts ``{ids, changes}`` (frontend) or
  ``[{id, ...}]`` (legacy); same union response.
- ``POST bulk-archive/`` / ``POST bulk-restore/``: accepts ``{ids}``;
  union response with ``summary.archivedCount|restoredCount``.
- ``POST {id}/archive|restore/``: single-entity soft-delete / restore,
  responding with the entity detail serializer.

Concrete viewsets set ``detail_serializer_class`` and extend
``permission_map`` with the action names they expose.
"""
...
class BulkContractViewSetMixin:
    ...
    def _clean_create_data(self, data):
        ...
        Drops client-supplied identity/ownership keys (``id``,
        ``organization``/``organization_id``) — ownership is always derived
        server-side from the resolved organization + project.
        """
        model = self.service_class.model
        allowed = set()
    ...
    def _set_status(self, instance, value):
        """Flip ``status`` when the model supports the value (mock parity:
        archive -> 'Archived', restore -> 'In Progress')."""
        try:
