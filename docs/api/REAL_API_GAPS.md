...
| AI chat / risks / summaries | `AIService.ts` (+ mock fallback), `useAIWorkspace` | DOCUMENTED-STUB; REAL = LLM+index epic (roadmap) | No LLM/index infra; code already labeled explicit stub (`ai.py:1-6`) |
...
| Reports / notifications | `organizationApi.ts:595,600` | DOCUMENTED-STUB now (frontend empty-states + contract note) or REAL P2 | No models exist; empty renders today |
